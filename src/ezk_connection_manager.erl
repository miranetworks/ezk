...
set_connection_monitor(PidOrProcessName,Message) ->
    gen_server:call(?SERVER, {set_connection_monitor, PidOrProcessName, Message}).
...
handle_call({start_connection, Servers, MonitorPIds}, _From, #con_man_state{connection_monitor=Notifier}=State) ->
    case Servers of
	[] ->
	...
	{ok, ConnectionPId} ->
            case Notifier of
                undefined       -> ok;
                {Proc, Message} -> Proc ! {Message, ConnectionPId}
            end,
	    ?LOG(1,"Started"),
...
handle_call({set_connection_monitor, PidOrProcessName, Message}, _From, State) ->
    {reply, ok, State#con_man_state{connection_monitor={PidOrProcessName, Message}}}.
