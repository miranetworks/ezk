...
-export([set_connection_monitor/1]).
...
set_connection_monitor(PidOrProcessName) ->
    gen_server:call(?SERVER, {set_connection_monitor, PidOrProcessName}).
...
handle_call({start_connection, Servers, MonitorPIds}, _From, #con_man_state{connection_monitor=Notifier}=State) ->
	...
	{ok, ConnectionPId} ->
            case Notifier of
                undefined -> ok;
                _         -> Notifier ! {add_connection, ConnectionPId}
            end,
	    ?LOG(1,"Started"),
...
handle_call({set_connection_monitor, PidOrProcessName}, _From, State) ->
    {reply, ok, State#con_man_state{connection_monitor=PidOrProcessName}}.
